...
def booking_manager():
    ...
    with st.expander("➕ Add New Booking"):
        with st.form("new_b"):
            ...
            b_start = c4.selectbox("Start", [f"{h:02d}:{m:02d}" for h in range(6, 24) for m in [0, 30]], index=28)
            b_end = c5.selectbox("End", [f"{h:02d}:{m:02d}" for h in range(6, 24) for m in [0, 30]], index=30)
            ...
            with st.expander("➕ New Student"):
                with st.form("reg"):
                    ...
                    if st.form_submit_button("Register"):
                        ...
                        new_s = pd.DataFrame([{"ID": nid, "Name": name, "Mobile Number": mob, "Active": "TRUE", "Coaching Fee": 3000}])
                        save_data("Student Details", pd.concat([s_df, new_s]))
                        st.rerun()
            for _, row in s_df.iterrows():
                with st.container(border=True):
                    ci, cb = st.columns([4,1])
                    status = "🟢" if str(row['Active']).upper() == 'TRUE' else "🔴"
                    ci.write(f"{status} **{row['Name']}** | {row['Mobile Number']}")
                    if cb.button("Edit", key=row['ID']):
        ...
        if not active_s.empty:
            with st.form("fee"):
                ...
                if st.form_submit_button("Pay & Send Receipt"):
                    ...
                    save_data("Fees Payment", pd.concat([f_df, new_p]))
                    txt = urllib.parse.quote(f"🧾 *FEE RECEIPT*\nName: {sel_name}\nMonth: {month}\nAmount: ₹{amt}\nStatus: Paid")
                    st.link_button("📲 WhatsApp Receipt", f"https://wa.me/{clean_phone(sel_row['Mobile Number'])}?text={txt}")
